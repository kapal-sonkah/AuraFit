...
function durasi(keterangan = '') {
  const m = String(keterangan ?? '').match(/(\d+)\s*menit/i);
  ...
  if (!berikutnya) {
    return (
      <section className="next-step next-step--done" aria-labelledby="next-step-title">
        ...
        <p className="next-step__meta">Streak aman. Aktivitas tambahan juga menjaga streak.</p>
      </section>
    );
  }
  ...
  return (
    <section className="next-step" aria-labelledby="next-step-title">
      ...
      <p className="next-step__meta">
        {lama ? `${lama} · ` : ''}Satu aktivitas selesai menjaga streak hari ini. Aktivitas tambahan juga ikut dihitung.
      </p>
